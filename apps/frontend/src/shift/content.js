 ...
 *
 * This is the editorial reference AND the offline fallback: `useDomains` prefers
 * the live `/api/map` document and falls back to this when the API is
 * unavailable, so the site is never blank.
 *
 * Shifts carry their page composition as a `modules` array — the same
 * `{ type, data }` contract the pipeline emits (see kt_modules / st_modules in
 * generate_map_data.py, and packages/contracts/shift_modules.json). Authoring it
 * as data rather than deriving it here means there is no second copy of the
 * template to drift out of step with the backend.
 *
 * The design ships a full module set for one shift ("Cognitive Erosion") and a
 * dek only for the rest, which is exactly what its own conditional sections do.
 */

/** A headline-only shift: hero + standfirst, which is all the design gives it. */
const headline = (s) => ({ ...s, modules: [{ type: 'dek', data: { text: s.dek } }] })
...
// ── The fully-authored shift ────────────────────────────────────────────────
// "COGNITIVE EROSION" is the reference template: every module the shift page
// supports, in the order the reader meets them.

const COGNITIVE_EROSION_DEK =
  'AI is systematically degrading the human capacities — clear reasoning, shared understanding, intellectual independence — that modern democracy and markets depend on.'

const SUB_CAPACITY_COLLAPSE = {
  ...
  modules: [
    { type: 'lede', data: { text: 'The reasoning and literacy decline AI is being absorbed into predates it — and AI is accelerating what was already in motion.' } },
    { type: 'from_to_solid', data: {
    ...
    { type: 'counter_signals', data: { items: [
...
const SUB_GOVERNANCE_GAP = {
  ...
  modules: [
    { type: 'lede', data: { text: 'Rules written for one speed of change are being applied to three, and the gaps between them are where the damage accumulates.' } },
    { type: 'from_to_solid', data: {
...
const SUB_EPISTEMIC_DRIFT = {
  ...
  modules: [
    { type: 'lede', data: { text: 'The way we talk about AI is training people out of the reasoning they would need to think about it clearly.' } },
    { type: 'from_to_solid', data: {
    ...
    { type: 'tension_band', data: { label: 'The tension', quote: 'The loudest analysis of AI is the least analytical, and it is winning.' } },
    { type: 'stat_band', data: {
      value: '2:1',
      text: 'Content framing AI in absolute terms — total salvation or total catastrophe — consistently outperforms qualified analysis by roughly two to one on engagement, selecting for the least useful framing of the most consequential topic.',
      source: 'Serious Shift analysis · platform engagement patterns',
    } },
    { type: 'peel_tabs', data: {
      whats_changing: 'This is not a story about misinformation. It is a story about form. The formats where AI is discussed most — short video, threads, headline aggregation — structurally reward confidence and punish qualification, so the arguments that spread are the ones that concede nothing. Over time that does not just distort what people believe about AI; it degrades the habit of holding two possibilities at once, which is precisely the habit required to make good decisions about it.',
      why_now: 'AI has become a mass-culture topic at the exact moment discourse infrastructure is at its most engagement-optimised. There is no longer a slower, more deliberate tier of public conversation that the fast tier eventually answers to. The first draft is the only draft, and it is written for velocity.',
    } },
    { type: 'human_needs', data: {
...
const SUB_OVERSIGHT_ASYMMETRY = {
  ...
  modules: [
    { type: 'lede', data: { text: 'Safety constraints apply to the people who follow rules. The asymmetry that creates compounds as capability rises.' } },
    { type: 'from_to_solid', data: {
    ...
    { type: 'tension_band', data: { label: 'The tension', quote: 'Every guardrail is a rule that only the compliant obey.' } },
    { type: 'stat_band', data: {
      value: '100%',
      text: 'In documented AI-assisted intrusions to date, the attacking systems operated without safety constraints while every defending system was operating under them — an asymmetry present in every case, not a subset.',
      source: 'Serious Shift analysis · published incident reporting',
    } },
    { type: 'peel_tabs', data: {
      whats_changing: 'Guardrails were designed as a property of models. In practice they are a property of institutions: the organisations that install them are the ones already inclined to behave well. As capability rises, the gap between a constrained defender and an unconstrained attacker does not stay constant — it widens, because the same capability increase buys more freedom on the unconstrained side. Defence is being asked to win a fight it enters with a handicap that grows with every model release.',
      why_now: 'Capable open-weight models are now widely available, and the tooling to strip constraints is trivial. At the same time, regulated sectors are formalising the constraints their own defensive systems must operate under. Both curves are steepening at once, in opposite directions.',
    } },
    { type: 'human_needs', data: {
...
const SUB_UNDERSTANDING_PREMIUM = {
  ...
  modules: [
    { type: 'lede', data: { text: 'As competence becomes cheap to simulate, the ability to actually understand something turns into a priced, scarce and defensible asset.' } },
    { type: 'from_to_solid', data: {
...
const COGNITIVE_EROSION = {
  id: 's5',
  kicker: 'Key shift 11',
  title: '“COGNITIVE EROSION”',
  read: '5 min read',
  dek: COGNITIVE_EROSION_DEK,
  subshifts: [
    SUB_CAPACITY_COLLAPSE, SUB_GOVERNANCE_GAP, SUB_EPISTEMIC_DRIFT,
    SUB_OVERSIGHT_ASYMMETRY, SUB_UNDERSTANDING_PREMIUM,
  ],
  modules: [
    { type: 'dek', data: { text: COGNITIVE_EROSION_DEK } },
    { type: 'from_to', data: {
    ...
    { type: 'peel_tabs', data: {
      whats_changing: "Quality used to be scarce. AI made it free. Polished content, convincing images, competent writing, on demand, for nothing. That doesn't kill the desire for quality. It kills quality as a differentiator. What's scarce now is proof that a human chose to make something. Spent the hours, picked the materials, left their mark. Consumers aren't being sentimental about craft. They're doing what consumers always do: paying a premium for whatever's hardest to get.",
      why_now: 'Three things landed at once: assistants moved from novelty to default in schools and offices, the first cohort educated alongside them entered the workforce, and the measurement caught up — literacy and comprehension scores are now falling in the exact places where AI adoption is highest. The rebound market forms while the damage is still deniable.',
    } },
    ...
    { type: 'tension_band', data: { quote: 'I use AI because it makes me more capable — but I’m starting to suspect it’s making me less capable.' } },
    { type: 'timeline', data: { steps: [
