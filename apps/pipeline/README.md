...
| `GEMINI_API_KEY` | no | Generates each shift's artwork during synthesis, published in the same commit as the map. Unset disables generation entirely: the run publishes with whatever is already in `shift_art`, falling back to the committed vector posters. Never fatal — art is decoration, the taxonomy is content. |
| `SS_ART` / `SS_ART_FORCE` / `SS_ART_WORKERS` / `SS_ART_BUDGET_USD` / `SS_ART_DEADLINE_SECONDS` / `SS_ART_MODEL` | no | Kill switch, forced regeneration, concurrency, runaway ceiling, wall-clock deadline, model override. |
| `SS_KT_CHANGE_BUDGET` | no | How many key shifts one sphere should expect to rename in a week. A target expressed in the prompt, deliberately not a gate — a run never fails for exceeding it. Default 2. |
